...
const INPUT_DRAWS: [u32; 100] = [
    ...
    85, 80, 30, 36, 47, 3, 26, 57, 84, 25, 63, 27, 37, 94, 7, 45, 58, 9, 78, 65, 72, 6, 14, 61, 20,
    1, 42, 89,
];

const INPUT_BOARDS: [&str; 599] = [
    "83 11 47 61 45",
    "30 74 73 14 66",
    "53 52 10 57 15",
    "64 50 54 28 87",
...
impl Board {
    ...
    fn try_mark(&mut self, drawn_number: u32) {
        for r in 0..5 {
            for c in 0..5 {
                if self.grid[r][c] == drawn_number {
    ...
    fn has_won(&self) -> bool {
        ...
        'columns_check: for c in 0..5 {
            for r in 0..5 {
                if !self.marked[r][c] {
                    continue 'columns_check;
                }
            }
            return true;
        }
...
fn parse_boards(input_boards: &[&str]) -> Vec<Board> {
    ...
    loop {
        ...
        i += 6; // 5 rows of the just parsed board + 1 empty line.
        if i >= input_boards.len() {
            break boards;
        }
    }
}
...
pub fn solution_1() -> String {
    let mut boards = parse_boards(&INPUT_BOARDS);

    for drawn_number in INPUT_DRAWS {
        for board in &mut boards {
            board.try_mark(drawn_number);
        }

        for board in &boards {
            if board.has_won() {
                return (board.sum_of_unmarked() * drawn_number).to_string();
            }
        }
    }
    panic!("No board won");
    ...
    for drawn_number in INPUT_DRAWS {
        ...
        for (i, board) in boards.iter().enumerate() {
            ...
            if board.has_won() {
                ...
                // Because all numbers will be eventually drawn, all boards will eventually win.
                // Therefore the last board to win wins after all other boards have won.
                if won.iter().all(|&w| w == true) {
                    return (board.sum_of_unmarked() * drawn_number).to_string();
                }
            }
        }
