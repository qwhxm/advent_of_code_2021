...
impl FromStr for Entry {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        Ok(Entry {
            all_digits: s_split[0]
                .split_whitespace()
                .map(|s| s.chars().collect())
                .collect(),
            output_digits: s_split[1]
                .split_whitespace()
                .map(|s| s.chars().collect())
                .collect(),
        })
...
pub fn solution_1() -> String {
    let entries = INPUT.iter().map(|&e| Entry::from_str(e).unwrap());

    let mut num_1s_4s_7s_and_8s = 0;
    for entry in entries {
        for output_digit in entry.output_digits {
            match output_digit.len() {
                2 | 4 | 3 | 7 => num_1s_4s_7s_and_8s += 1,
                _ => (),
            }
        }
    }
...
fn deduce_digits(all_digits: Vec<Digit>) -> [Digit; 10] {
    ...
    let deduced_7 = &digits_by_num_segments[&3][0];

    // 4 is the only digit with 4 segments.
    let deduced_4 = &digits_by_num_segments[&4][0];

    // 8 is the only digit with 7 segments.
    let deduced_8 = &digits_by_num_segments[&7][0];

    // From the 6-segment digits,
    // ...9 is the only one that contains all segments of 4;
    let deduced_9 = digits_by_num_segments[&6]
        .iter()
        .find(|d| deduced_4.is_subset(d))
        .unwrap();

    // ...6 is the only one that doesn't contain all segments of 7;
    let deduced_6 = digits_by_num_segments[&6]
        .iter()
        .find(|d| !deduced_7.is_subset(d))
        .unwrap();
    ...
    // ...3 is the only one that contains all segments of 7;
    let deduced_3 = digits_by_num_segments[&5]
        .iter()
        .find(|d| deduced_7.is_subset(d))
        .unwrap();

    // ...5 is the only one whose segments are all contained in 6;
    let deduced_5 = digits_by_num_segments[&5]
        .iter()
        .find(|d| d.is_subset(deduced_6))
        .unwrap();
...
pub fn solution_2() -> String {
    ...
    for entry in entries {
        let deduced_digits = deduce_digits(entry.all_digits);
    ...
    output_values.iter().sum::<u32>().to_string()
}
